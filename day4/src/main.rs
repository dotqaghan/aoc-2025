...
fn main() {
    ...
    let mut data: Vec<Vec<i32>> = file
        .lines()
        .map(|line| line.chars().map(|ch| if ch == '@' {1} else {0}).collect())
        .collect();
...
fn count_surrounding(data: &Vec<Vec<i32>>, x: usize, y: usize) -> i32 {
    ...
    count += if x > 0 && y > 0     { data[y - 1][x - 1] } else { 0 };
    count += if x > 0              { data[y][x - 1] } else { 0 };
    count += if x > 0 && y < y_max { data[y + 1][x - 1] } else { 0 };

    count += if y > 0     { data[y - 1][x] } else { 0 };
    count += if y < y_max { data[y + 1][x] } else { 0 };

    count += if x < x_max && y > 0     { data[y - 1][x + 1] } else { 0 };
    count += if x < x_max              { data[y][x + 1] } else { 0 };
    count += if x < x_max && y < y_max { data[y + 1][x + 1] } else { 0 };
