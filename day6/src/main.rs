...
fn part1(input: &str) -> i64 {
    let parsed: Vec<Vec<&str>> = input.lines().map(|line| line.split_whitespace().collect::<Vec<_>>()).collect();
    let numbers: Vec<Vec<i64>> = (0..parsed[0].len())
        .map(|col| { (0..parsed.len() - 1)
            .map(|row|
                parsed[row][col].parse().unwrap()
            ).collect()
        }).collect();

    numbers.iter().enumerate().map(|(i, v)|
        match parsed[parsed.len() - 1][i] {
            "*" => v.iter().product::<i64>(),
            "+" => v.iter().sum::<i64>(),
            _ => unreachable!(),
        }).sum()
...
fn part2(input: &str) -> Vec<i64> {
    ...
    let numbers: Vec<String> = (0..parsed[0].len()).map(|col| unsafe {
        let x = (0..parsed.len() - 1).map(|row| parsed[row][col]).collect::<Vec<_>>();
    ...
    let results: Vec<i64> = numbers.iter().enumerate().map(|(i, v)| {
